...
class Walletrechargerecord extends Base
{
    CONST COMMAND_NAME = 'phpnsq:sub1';
                ...
                if($bodyArr['sysId'] == 'TEST'){
					...
					if($bodyArr['coinId'] == 102){
						...
						if(!mysqli_fetch_array($res)){
							...
							$info = '充币到账';
							//写流水记录
							mysqli_query($db, "INSERT INTO tp_usdt_log (user_id,`quantity`,`hash`,`timestamp`,`type`,`info`,create_time) VALUES({$user_id},{$quantity},'{$hash}','{$timestamp}',{$type},'{$info}',{$create_time});");
							mysqli_query($db, "UPDATE tp_users SET usdt = usdt + {$quantity} WHERE user_id = {$user_id}");
							mysqli_close($db);
							//回调
							//$url = "http://120.24.172.245:3000/coin/recharge/callback";
						}
					}
